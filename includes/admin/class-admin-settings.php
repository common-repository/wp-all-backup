...
// End Fixed Vulnerability 22-06-2016 for prevent direct download
    ...
    function wp_all_backup_create_archive() {
        ...
        //fclose(fopen($path_info['basedir'] . '/' . WPALLBK_BACKUPS_DIR .'/.htaccess', $htassesText));
 ...
 RewriteRule . - [R=403,L]");
      ...
      //  error_log("number_of_backups_from_user");
       // error_log($number_of_backups_from_user);
        ...
        if (get_option('wp_all_backup_type') == 'complete') {
        ...
        $methodZip = 0;
        if ($this->get_zip_command_path() && (get_option('wp_all_backup_type') == 'File' || get_option('wp_all_backup_type') == 'complete')) {
            if (!$this->zip($path_info['basedir'] . '/' . WPALLBK_BACKUPS_DIR . '/' . $WPDBFileName)) {
                $methodZip = 1;
             //   error_log('Error : zip');
            }
        } else {
        ...
        if ($methodZip == 0) {
          //  error_log("Zip method: Zip cmd");
            $logMessage.="\n Exclude Folders and Files : " . get_option('wp_all_backup_exclude_dir');
          ...
          //  error_log("Class ZipArchive Not Present");
            ...
            $arcname = $path_info['basedir'] . '/' . WPALLBK_BACKUPS_DIR . '/' . $WPDBFileName;
            ...
            if (get_option('wp_all_backup_type') == 'Database') {
                $filename = $FileName . '.sql';
    ...
    function wp_all_backup_create_installer($filename) {
        return  '';
        //pro version
        $content = file_get_contents(WPALLBK_PLUGIN_DIR . 'includes/admin/template/template_install.txt');
    ...
    public function wpallbk_update_backup_info($FileName, $logFile, $logMessage = '') {
        ...
        $upload_path = array(
            ...
            'size' => ($filesize),
            'type' => get_option('wp_all_backup_type')
        );


        if (get_option('wp_all_backup_enable_log') == 1) {
            $this->write_log($logFile, $logMessage);
            $upload_path['logfile'] = $path_info['baseurl'] . '/' . WPALLBK_BACKUPS_DIR . '/log/' . $FileName . '.txt';
